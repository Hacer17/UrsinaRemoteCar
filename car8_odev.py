    ...
    def input(self, key):
        ...
        if key == "middle mouse down" and mouse.world_point and self.clicked:
            for rampa, kopru in zip(rampalar, kopruler):
                ...
                rampa.collider = kopru.collider = "box"
        ...
        if key == "backspace" and self.select:
            try:
                self.lines.model.vertices.pop(Point.index)
                self.lines.model.generate()
            except: pass
...
class Point(Entity):
    ...
    def input(self, key):
        ...
        if key == "backspace" and self.color == color.lime:
            try:
                Point.pointList.remove(self)
                destroy(self)
            except: pass
